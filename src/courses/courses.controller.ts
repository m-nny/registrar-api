...
class CourseController implements Controller {
    ...
    private initializeRoutes() {
        ...
        this.router.patch(
            `${this.path}/:id`,
            validationMiddleware(CreateCourseDto, true),
            this.modifyCourse,
        );
    ...
    private getCourseById: express.RequestHandler = (request, response, next) => {
        ...
        this.Course.findById(id).then(course => {
            if (course) {
    ...
    private modifyCourse: express.RequestHandler = (request, response, next) => {
        ...
        this.Course.findByIdAndUpdate(id, courseData, { new: true }).then(course => {
            if (course) {
                response.send(course);
            } else {
                next(new PostNotFoundException(id));
    ...
    private createCourse: express.RequestHandler = (request, response) => {
        ...
        createdCourse.save().then(savedCourse => {
    ...
    private deleteCourse: express.RequestHandler = (request, response, next) => {
        const { id } = request.params;
        this.Course.findByIdAndDelete(id).then(successResponse => {
            if (successResponse) {
